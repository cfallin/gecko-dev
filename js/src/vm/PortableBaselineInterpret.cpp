...
uint64_t MOZ_ALWAYS_INLINE ICInterpretOps(uint64_t arg0, uint64_t arg1,
...
#define FAIL_IC()                            \
  do {                                       \
    ctx.error = ICInterpretOpResult::NextIC; \
...

        {
          ...
          if (isNative) {
            ...
            switch (PortableBaselineInterpret(
                ...
                reinterpret_cast<Value*>(&retValue))) {
              case PBIResult::Ok:
                break;
              ...
              case PBIResult::Unwind:
                ctx.error = ICInterpretOpResult::Unwind;
                return IC_ERROR_SENTINEL();
              case PBIResult::UnwindError:
                ctx.error = ICInterpretOpResult::UnwindError;
                return IC_ERROR_SENTINEL();
              case PBIResult::UnwindRet:
                ctx.error = ICInterpretOpResult::UnwindRet;
                return IC_ERROR_SENTINEL();
            }
          }
        }
  ...
  static uint64_t MOZ_ALWAYS_INLINE IC##kind(uint64_t arg0, uint64_t arg1, \
    ...
    next_stub:                                                             \
      ...
      error:                                                               \
        ...
        retValue = ICInterpretOps(arg0, arg1, stub, ctx);                  \
        if (retValue != IC_ERROR_SENTINEL()) {                             \
          return retValue;                                                 \
        }                                                                  \
        if (ctx.error == ICInterpretOpResult::NextIC) {                    \
          stub = stub->maybeNext();                                        \
          goto next_stub;                                                  \
        }                                                                  \
        return retValue;                                                   \
...
#ifdef TRACE_INTERP
  {
    ...
    generic_cmp: {
      ...
      CASE(CheckPrivateField) {
        ...
        IC_ZERO_ARG(2);
        INVOKE_IC(CheckPrivateField, false);
        IC_PUSH_RESULT();
      ...
      CASE(OptimizeSpreadCall) {
        ...
        IC_ZERO_ARG(1);
        IC_ZERO_ARG(2);
        INVOKE_IC(OptimizeSpreadCall, false);
        IC_PUSH_RESULT();
      ...
      CASE(OptimizeGetIterator) {
        ...
        IC_ZERO_ARG(1);
        IC_ZERO_ARG(2);
        INVOKE_IC(OptimizeGetIterator, false);
        IC_PUSH_RESULT();
