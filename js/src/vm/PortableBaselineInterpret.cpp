...
#define PREDICT_RETURN()                                 \
  ...
  // Save inputs so a stub failure can restore them.
  ...
  CACHEOP_CASE(GuardToObject) {
    ...
    ctx.icregs.icVals[inputId.id()] = reinterpret_cast<uint64_t>(&v.toObject());
  ...
  CACHEOP_CASE(GuardToString) {
    ...
    ctx.icregs.icVals[inputId.id()] = reinterpret_cast<uint64_t>(v.toString());
  ...
  CACHEOP_CASE(GuardToSymbol) {
    ...
    ctx.icregs.icVals[inputId.id()] = reinterpret_cast<uint64_t>(v.toSymbol());
  ...
  CACHEOP_CASE(GuardToBigInt) {
    ...
    ctx.icregs.icVals[inputId.id()] = reinterpret_cast<uint64_t>(v.toBigInt());
    DISPATCH_CACHEOP();
  }
  ...
  CACHEOP_CASE(GuardShape) {
    ...
    JSObject* obj = reinterpret_cast<JSObject*>(ctx.icregs.icVals[objId.id()]);
  ...
  CACHEOP_CASE(HasClassResult) {
    ...
    JSClass* clasp = reinterpret_cast<JSClass*>(
        cstub->stubInfo()->getStubRawWord(cstub, claspOffset));
    ctx.icregs.icResult = BooleanValue(obj->getClass() == clasp).asRawBits();
    PREDICT_RETURN();
    DISPATCH_CACHEOP();
  }
  ...
  CACHEOP_CASE(GuardStringToIndex) {
    ...
    Int32OperandId resultId = cacheIRReader.int32OperandId();
    BOUNDSCHECK(resultId);
    JSString* str = reinterpret_cast<JSString*>(ctx.icregs.icVals[strId.id()]);
    int32_t result;
  ...
  CACHEOP_CASE(GuardStringToNumber) {
    ...
    if (str->hasIndexValue()) {
      ...
      double value;
      if (!StringToNumberPure(ctx.frameMgr.cxForLocalUseOnly(), str, &value)) {
        FAIL_IC();
      }
  ...
  CACHEOP_CASE(GuardHasGetterSetter) {
    ...
    if (!ObjectHasGetterSetterPure(ctx.frameMgr.cxForLocalUseOnly(), obj, id,
                                   getterSetter)) {
      FAIL_IC();
    }
    DISPATCH_CACHEOP();
  ...
  CACHEOP_CASE(GuardFunctionHasJitEntry) {
    ...
    JSObject* fun = reinterpret_cast<JSObject*>(ctx.icregs.icVals[funId.id()]);
  ...
  CACHEOP_CASE(MegamorphicLoadSlotResult) {
    ...
    if (!GetNativeDataPropertyPureWithCacheLookup(
            ctx.frameMgr.cxForLocalUseOnly(), obj, name, nullptr, &result)) {
      FAIL_IC();
    }
  ...
  CACHEOP_CASE(MegamorphicLoadSlotByValueResult) {
    ...
    Value values[2] = {id};
    if (!GetNativeDataPropertyByValuePure(ctx.frameMgr.cxForLocalUseOnly(), obj,
                                          nullptr, values)) {
      FAIL_IC();
    }
  ...
  CACHEOP_CASE(StoreDenseElementHole) {
    ...
    if (index < initLength) {
      ...
      if (index >= nobj->getDenseCapacity()) {
        if (!NativeObject::addDenseElementPure(ctx.frameMgr.cxForLocalUseOnly(),
                                               nobj)) {
          FAIL_IC();
        }
      }
  ...
  CACHEOP_CASE(LinearizeForCharAccess) {
    ...
    if (!str->isRope()) {
      ...
      ctx.icregs.icVals[resultId.id()] = reinterpret_cast<uintptr_t>(result);
    }
  ...
  CACHEOP_CASE(LoadObjectResult) {
    ObjOperandId objId = cacheIRReader.objOperandId();
    ctx.icregs.icResult =
        ObjectValue(*reinterpret_cast<JSObject*>(ctx.icregs.icVals[objId.id()]))
            .asRawBits();
    PREDICT_RETURN();
    DISPATCH_CACHEOP();
  }
  ...
  CACHEOP_CASE(LoadStringResult) {
    StringOperandId strId = cacheIRReader.stringOperandId();
    ctx.icregs.icResult =
        StringValue(reinterpret_cast<JSString*>(ctx.icregs.icVals[strId.id()]))
            .asRawBits();
    PREDICT_RETURN();
    DISPATCH_CACHEOP();
  }
  ...
  CACHEOP_CASE(LoadInt32Result) {
    ...
    ctx.icregs.icResult = Int32Value(ctx.icregs.icVals[valId.id()]).asRawBits();
    PREDICT_RETURN();
    DISPATCH_CACHEOP();
  }
...
#define INT32_OP(name, op, extra_check)                            \
  ...
  CACHEOP_CASE(LoadValueResult) {
    uint32_t valOffset = cacheIRReader.stubOffset();
    ctx.icregs.icResult = cstub->stubInfo()->getStubRawInt64(cstub, valOffset);
    PREDICT_RETURN();
    DISPATCH_CACHEOP();
  }
  ...
  CACHEOP_CASE(CallStringConcatResult) {
    ...
    FakeRooted<JSString*> lhs(
        nullptr, reinterpret_cast<JSString*>(ctx.icregs.icVals[lhsId.id()]));
    FakeRooted<JSString*> rhs(
        nullptr, reinterpret_cast<JSString*>(ctx.icregs.icVals[rhsId.id()]));
    JSString* result =
...
#define INVOKE_IC(kind)                                              \
  ctx.pc = pc;                                                       \
  ctx.sp = sp;                                                       \
  CALL_IC(ctx, frame->interpreterICEntry()->firstStub(), ic_result); \
  ...
  generic_cmp: {
    ...
    CASE(ToString) {
      ...
      }
      {
        ...
        if (JSString* result =
                ToStringSlow<NoGC>(ctx.frameMgr.cxForLocalUseOnly(), value0)) {
